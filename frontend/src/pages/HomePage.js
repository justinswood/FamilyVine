...
import { Github, Calendar, MapPin, BookOpen, ArrowRight, Camera, BookOpenText, Users } from 'lucide-react';
...
const HomePage = () => {
  ...
  const latestAlbum = useMemo(() => {
    ...
    return [...allAlbums].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];
  }, [allAlbums]);

  const latestStory = useMemo(() => {
    if (!allStories?.length) return null;
    return [...allStories].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];
  }, [allStories]);
  ...
  return (
    <div className="homepage-heritage min-h-screen">
      ...
      {/* Heritage Corner — Recent Activity */}
      <div className="heritage-corner-section">
        <h2 className="heritage-corner-header">
          <span className="heritage-corner-rule" />
          <span className="heritage-corner-title">Heritage Corner</span>
          <span className="heritage-corner-rule" />
        </h2>

        <div className="heritage-corner-grid">
          {/* Panel 1: Latest Album */}
          <div className="heritage-card">
            <div className="heritage-card-icon">
              <Camera className="w-5 h-5" />
            </div>
            <h3 className="heritage-card-label">Latest Album</h3>
            {latestAlbum ? (
              <>
                {latestAlbum.cover_photo_path && (
                  <div className="heritage-card-thumb">
                    <img
                      src={`${process.env.REACT_APP_API}/${latestAlbum.cover_photo_path}`}
                      alt={latestAlbum.title}
                      loading="lazy"
                    />
                  </div>
                )}
                <h4 className="heritage-card-title">{latestAlbum.title}</h4>
                <p className="heritage-card-meta">{latestAlbum.photo_count} photo{latestAlbum.photo_count !== 1 ? 's' : ''}</p>
                <Link to="/gallery" className="heritage-card-link">
                  View Album <ArrowRight className="w-3.5 h-3.5 inline" />
                </Link>
              </>
            ) : (
              <p className="heritage-card-empty">No albums yet</p>
            )}
          </div>

          {/* Panel 2: Recent Story */}
          <div className="heritage-card">
            <div className="heritage-card-icon heritage-card-icon-story">
              <BookOpenText className="w-5 h-5" />
            </div>
            <h3 className="heritage-card-label">Recent Story</h3>
            {latestStory ? (
              <>
                <h4 className="heritage-card-title">{latestStory.title}</h4>
                {latestStory.content && (
                  <p className="heritage-card-excerpt">
                    {latestStory.content.replace(/<[^>]*>/g, '').substring(0, 120)}...
                  </p>
                )}
                {latestStory.author_name && (
                  <p className="heritage-card-meta">by {latestStory.author_name}</p>
                )}
                {latestStory.members?.length > 0 && (
                  <div className="heritage-card-avatars">
                    {latestStory.members.slice(0, 3).map(m => (
                      <ProfileImage key={m.id} member={m} size="w-6 h-6" className="rounded-full" />
                    ))}
                  </div>
                )}
                <Link to="/stories" className="heritage-card-link">
                  Read More <ArrowRight className="w-3.5 h-3.5 inline" />
                </Link>
              </>
            ) : (
              <p className="heritage-card-empty">No stories yet</p>
            )}
          </div>

          {/* Panel 3: Newest Members */}
          <div className="heritage-card">
            <div className="heritage-card-icon heritage-card-icon-members">
              <Users className="w-5 h-5" />
            </div>
            <h3 className="heritage-card-label">Newest Members</h3>
            {newestMembers.length > 0 ? (
              <>
                <div className="heritage-card-members">
                  {newestMembers.map(m => (
                    <Link key={m.id} to={`/members/${m.id}`} className="heritage-member-pip" title={`${m.first_name} ${m.last_name}`}>
                      <ProfileImage member={m} size="w-10 h-10" className="rounded-full" />
                    </Link>
                  ))}
                </div>
                <div className="heritage-card-names">
                  {newestMembers.map(m => (
                    <span key={m.id} className="heritage-member-name">{m.first_name}</span>
                  ))}
                </div>
                <Link to="/members" className="heritage-card-link">
                  View All Members <ArrowRight className="w-3.5 h-3.5 inline" />
                </Link>
              </>
            ) : (
              <p className="heritage-card-empty">Add your first member</p>
            )}
          </div>
        </div>
      </div>
